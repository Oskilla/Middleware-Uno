...
public class Main {
	...
	private void CreationPaquet() {
		...
		for(cptCouleurs= 0;cptCouleurs <4; cptCouleurs ++ ) {
			...
			paquet.add(new Carte(new Figure(Figures[cptFigure]),0,Couleurs[cptCouleurs]));
			for(int cptNum =0; cptNum < 18; cptNum ++) {
				//Comme l'on doit crée 2 fois chacune des num. On les passe mod 9 et on ajoute 1
				paquet.add(new Carte(new Figure(Figures[cptFigure]),(cptNum % 9)+1,Couleurs[cptCouleurs]));	
			}
			//creation des cartes sens interdit
			cptFigure += 1;
			paquet.add(new Carte(new Figure(Figures[cptFigure]),20,Couleurs[cptCouleurs]));
			paquet.add(new Carte(new Figure(Figures[cptFigure]),20,Couleurs[cptCouleurs]));
			...
			cptFigure+=1;
			paquet.add(new Carte(new Figure(Figures[cptFigure]),50,Couleurs[4]));
